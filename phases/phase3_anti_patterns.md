# Phase 3: 안티패턴 -- 절대 하지 말 것 (AP-01 ~ AP-33)
...
## [AP-21] 카드 장식 바(accent bar) 전면 금지 -- 상단/좌측 모두 금지

카드 상단 얇은 색 바, 좌측 세로 색 바 등 **모든 형태의 accent bar를 금지**한다.
"AI 생성물 티"의 대표 패턴이며, 좌측 세로 바 역시 동일한 문제를 가진다.

> **카드 디자인 원칙**: 카드는 **기본 평면 도형(사각형, 둥근사각형) + 그림자**만으로 구성한다.
> 입체감은 shadow로, 구분감은 색상 차이(CD/LT/WHITE)로 확보한다.

```
(X) 금지 패턴:
  1. 카드 상단 얇은 색 바 (accent bar on top)
  2. 카드 좌측 세로 색 바 (left vertical bar)
  3. 카드 외곽선 (border/outline)

(O) 허용 패턴:
  1. 둥근사각형(ROUNDED_RECTANGLE) + 그림자(shadow) -- 가장 기본, 최우선
  2. 밝은 카드(CD/LT/WHITE fill) + shadow -- 밝은 배경 본문용
  3. 어두운 카드(DOM/SEC fill) + 밝은 텍스트 -- 강조/다크 배경용
  4. 아이콘(원형 배경 + SVG)을 카드 내부 상단에 배치 -- 시각적 앵커
  5. 카드 내부 구분선(얇은 선)으로 제목/본문 구분 -- 허용
```
...
## [AP-28] 카드/배지 배경색과 텍스트 동일 톤 금지 -- 색상 대비 필수

카드·배지·블록의 배경색(fill)과 그 위의 텍스트 색상이 동일 톤(같은 색상 계열, 명도 차이 미미)이면 가독성이 심각하게 저하된다.

| 조합 | 판정 | 예시 |
|------|------|------|
| 골드 배경(B8963E) + 골드 텍스트(D4A847) | **NG** | 명도 차이 ~8%, 거의 안 보임 |
| 다크 배경(1A1A1A) + 골드 텍스트(D4A847) | **OK** | 명도 차이 ~60%, 선명 |
| DOM 배경(1B3A2D) + 흰 텍스트(FFFFFF) | **OK** | 명도 차이 ~75%, 완벽 |
| LT 배경(F5F3EE) + 회색 텍스트(CCCCCC) | **NG** | 명도 차이 ~12%, 흐릿함 |

**판정 기준**: 배경색과 텍스트색의 **상대 휘도(relative luminance) 대비비가 4.5:1 미만이면 위반** (WCAG AA).

**간편 체크**: 동일 색상 계열(금색-금색, 녹색-녹색, 회색-회색)은 무조건 의심. 반드시 **명/암 반전 조합**(어두운 배경+밝은 텍스트 또는 밝은 배경+어두운 텍스트)을 사용한다.

```
(X) 나쁜 예: palette.SEC 배경 + palette.ACC 텍스트 (둘 다 골드 계열)
(O) 좋은 예: palette.DK 배경 + palette.ACC 텍스트 (어두운 배경 + 밝은 강조색)
(O) 좋은 예: palette.LT 배경 + palette.DOM 텍스트 (밝은 배경 + 어두운 브랜드색)
```

**컴포넌트별 주의사항**:
- `highlight_body.js` dark-keywords 배지: fill은 반드시 `palette.DK`(어두운 색), value 텍스트는 `palette.ACC`(밝은 강조색)
- `card_grid.js` default 스타일: fill은 `LT/CD`(밝은 색), 타이틀은 `DOM/DK`(어두운 색)
- 모든 컴포넌트에서 fill과 color가 같은 palette 그룹(SEC+ACC, DOM+DK 등)이면 위반

---

## [AP-29] 동일 영역 내 요소 겹침(overlap) 금지 -- Y좌표 산술 검증 필수

Content Box 내에서 배치되는 요소들의 Y좌표가 겹치면 텍스트가 가려지거나 레이아웃이 무너진다. 특히 **하단 배지/카드 행 + Insight Box**처럼 바닥에 밀집되는 요소가 겹치기 쉽다.

**검증 공식**: `요소A의 (y + h) + gap(0.10") <= 요소B의 y` 가 참이어야 한다.

```
예시: 배지 카드 + 인사이트 박스 겹침 사고

(X) 겹침 발생:
  badgeY  = CB_Y + CB_H - 1.40 = 6.28   (h=1.20 → 하단 7.48)
  insightY = CB_Y + CB_H - 0.65 = 7.03   (h=0.60 → 하단 7.63)
  → 7.48 > 7.03: 0.45" 겹침!

(O) 수정 후:
  badgeY  = CB_Y + CB_H - 2.00 = 5.68   (h=1.20 → 하단 6.88)
  insightY = CB_Y + CB_H - 0.65 = 7.03   (h=0.60 → 하단 7.63)
  → 6.88 + 0.10 = 6.98 <= 7.03 ✓
```

**코드 생성 시 체크 절차**:
1. 슬라이드 내 모든 요소를 Y 오름차순으로 정렬
2. 인접 요소 쌍마다 `(위 요소 y + h) <= (아래 요소 y)` 확인
3. 위반 발견 시 위쪽 요소를 올리거나 아래 요소를 내려 gap 확보
4. Content Box 하단 경계(7.68") 초과 여부도 함께 확인

**자동 검증**: Phase 3.5 자체 검수에서 모든 본문·강조 슬라이드의 요소 Y좌표를 산술적으로 검증한다.

---

## [AP-30] 텍스트/Insight Box 겹침 절대 금지 -- 텍스트 영역 간 침범 제로 톨러런스

AP-29의 강화 버전. 특히 **텍스트 요소와 Insight Box, 카드 내부 텍스트와 구분선, 히어로 타이틀과 서브타이틀** 등 텍스트가 관여하는 모든 겹침을 절대 금지한다.

**핵심 원칙**: 텍스트 요소는 다른 어떤 요소와도 겹쳐서는 안 된다. 텍스트가 읽히지 않는 슬라이드는 존재 의미가 없다.

**검증 강화 포인트**:
1. **cover.js 히어로(42pt) + 서브타이틀**: 히어로의 실제 렌더링 높이를 계산하고, 서브타이틀 y가 히어로 하단보다 최소 0.15" 아래인지 확인
   - 한글 42pt 3줄: `42 × 1.25 × 3 / 72 = 2.19"` → h 최소 2.40"
   - 서브타이틀 y = 히어로 y + 히어로 h + 0.15"
2. **카드 내부 제목 + 구분선 + 본문**: 제목 h 이후 구분선 y, 구분선 이후 본문 y를 순차 검증
3. **Insight Box + 바로 위 콘텐츠**: Insight Box 위 0.10" 이내에 다른 요소가 없어야 함

```
(X) 극히 나쁜 예:
  히어로 y=2.91, h=1.80 → 하단 4.71
  서브타이틀 y=4.83     → 여유 0.12" (렌더링 시 겹침 발생!)
  
(O) 안전한 예:
  히어로 y=2.91, h=2.40 → 하단 5.31
  서브타이틀 y=5.46     → 여유 0.15" ✓
```

---

## [AP-31] addChart() 사용 금지 -- 도형 기반 수동 차트 필수

pptxgenjs의 `addChart()`는 PowerPoint 네이티브 차트 객체를 생성하여 **디자인 통제가 불가능**하다 (색상, 폰트, 라벨 위치가 PPT 테마에 종속). 모든 차트/그래프는 **addShape() + addText()로 수동 구축**한다.

**금지 함수**: `sl.addChart()`, `pptx.charts.*`

**도형 기반 차트 구축 패턴**:

### 막대 차트 (Bar Chart)
```javascript
// 값을 비율로 변환하여 막대 높이 결정
const maxVal = Math.max(...data.map(d => d.value));
const barMaxH = 3.5;  // 최대 막대 높이 (인치)
data.forEach((d, i) => {
  const barH = (d.value / maxVal) * barMaxH;
  const barY = baseY + barMaxH - barH;  // 아래 정렬
  sl.addShape(pptx.shapes.ROUNDED_RECTANGLE, {
    x: startX + i * (barW + gap), y: barY, w: barW, h: barH,
    fill: { color: d.color || palette.DOM },
    rectRadius: 0.04,
    line: { type: 'none' },
    shadow: sdw(),
  });
  // 값 라벨 (막대 상단)
  sl.addText(d.label, {
    x: startX + i * (barW + gap), y: barY - 0.25,
    w: barW, h: 0.25,
    fontSize: 10, fontFace: FN_MD, color: palette.TG,
    align: 'center', wrap: false, margin: 0,
  });
});
```

### 도넛 차트 (Donut Chart) -- 원호 근사
```javascript
// 원 배경 + 세그먼트 색상 원호 + 중앙 흰 원
// 정확한 원호는 어려우므로, 전체 원 + 중앙 원 + 콜아웃 라벨로 구성
sl.addShape(pptx.shapes.OVAL, {
  x: cx - r, y: cy - r, w: r * 2, h: r * 2,
  fill: { color: palette.DOM },
  line: { type: 'none' },
});
sl.addShape(pptx.shapes.OVAL, {  // 중앙 흰 원 (도넛 구멍)
  x: cx - ir, y: cy - ir, w: ir * 2, h: ir * 2,
  fill: { color: 'FFFFFF' },
  line: { type: 'none' },
});
// 콜아웃 라벨을 원 주변에 배치
...
### 비교 표 (Comparison Table) -- addChart() 대안
```javascript
// DOM 헤더 + 교차 행 배경 + 셀별 addText
const rowH = 0.35;
// 헤더
sl.addShape(pptx.shapes.RECTANGLE, {
  x: tblX, y: tblY, w: tblW, h: rowH,
  fill: { color: palette.DOM }, line: { type: 'none' },
});
// 데이터 행 (교차 배경)
rows.forEach((row, i) => {
  if (i % 2 === 0) {
    sl.addShape(pptx.shapes.RECTANGLE, {
      x: tblX, y: tblY + rowH * (i + 1), w: tblW, h: rowH,
      fill: { color: palette.CD }, line: { type: 'none' },
    });
  }
});
```
...
## [AP-32] 다크 배경 아이콘 -- 흰색 버전 필수

어두운 배경(DOM, DK, SEC 등) 위에 아이콘을 배치할 때, **반드시 흰색(FFFFFF) 버전 아이콘**을 사용한다. DOM/ACC 컬러 아이콘은 다크 배경에서 거의 보이지 않는다.

```javascript
// 아이콘 프리로드 시 흰색 버전 별도 생성
const ICONS = {
  car: await loadIcon('car-2.svg', palette.ACC),       // 밝은 배경용
  carW: await loadIcon('car-2.svg', 'FFFFFF'),         // 다크 배경용 (접미사 W)
  gauge: await loadIcon('gauge-1.svg', palette.DOM),   // 밝은 배경용
  gaugeW: await loadIcon('gauge-1.svg', 'FFFFFF'),     // 다크 배경용
};
```

**규칙:**
1. 밝은 배경(WHITE, CD, LT): palette.ACC 또는 palette.DOM 색상 아이콘
2. 어두운 배경(DOM, DK, SEC): 'FFFFFF' 흰색 아이콘 (접미사 W로 명명)
3. 아이콘 transparency: 다크 배경에서 최대 70% (85%는 보이지 않음)

---

## [AP-33] cover.js 3줄 이상 한글 히어로 -- 수동 빌드 필수

cover.js의 `_renderText`는 히어로 텍스트박스를 `h=1.80"`으로 고정하는데, **한글 42pt 3줄은 최소 2.19" 필요** (42 × 1.25 × 3 / 72). 이로 인해 3줄 이상 히어로에서 서브타이틀과 반드시 겹친다.

**판단 기준**: 히어로 텍스트가 2줄 이하이면 cover.js 사용 가능, **3줄 이상이면 수동 빌드 필수**.

```javascript
// 수동 빌드 예시 (Type B split cover, 3줄 히어로)
const splitX = SW * 0.45;
const heroH = 2.40;  // 3줄 × 42pt × 1.25 / 72 = 2.19" + 여유 0.21"
const subY = 2.91 + heroH + 0.15;  // = 5.46"

sl.addText('교통약자 이동의\n새로운 기준을\n세우겠습니다', {
  x: 0.57, y: 2.91, w: splitX - 0.57 - 0.30, h: heroH,
  fontSize: 42, fontFace: FN_XB, color: palette.DK,
  lineSpacingMultiple: 1.25, wrap: true,
});
sl.addText('서울시 복지재단 교통약자 이동지원 사업 제안', {
  x: 0.57, y: subY, w: splitX - 0.57 - 0.30, h: 0.45,
  fontSize: 14, fontFace: FN_TN, color: '888888',
});
```

**높이 계산 공식**:
```
히어로 h = ceil(글자수 / (텍스트박스 폭 / 한글1자폭)) × (fontSize × lineSpacing / 72) + 여유 0.20"
한글 1자 폭(인치) = fontSize × 0.6 / 72
```
